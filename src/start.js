...
function startAllInstancesAndAsgs({ dryRun, currentOperatingTimezone }) {
  ...
  return startAllInstances({
    ...
    .then((result) => {
      ...
      return spinUpOrResumeASGs({
        dryRun,
        currentOperatingTimezone,
        application: "all",
      });
      // If we only want to start asg for some applications
      // return spinUpOrResumeASGs({dryRun, currentOperatingTimezone, application: [
      //   "rqp-whics-wcf",
...
function spinUpASGs({ dryRun, currentOperatingTimezone, application }) {
  return listASGsToStart(currentOperatingTimezone, application).then(
    (startableASGs) => {
      if (dryRun) {
...
function resumeASGInstances({ dryRun, currentOperatingTimezone, application }) {
  return listASGsToResume(currentOperatingTimezone, application).then(
    (resumeableASGs) => {
      if (dryRun) {
        console.log("Dry run is enabled, will not start or untag any ASGs.");
        console.log(
        ...
        resumeableASGs.forEach((asg) => {
          ...
          const startedInstances = asg.Instances.map((insts) =>
            console.log(
              "Would have been started ec2 instance for suspended asg:",
              insts.InstanceId
            )
          );
